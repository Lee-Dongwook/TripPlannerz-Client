  ...
  return (
    <div>
          <List 
              ...
              renderItem={(item) => (
              <List.Item>
                <Card onClick={() => movetoSubPage(item.id? item.id : 0)} style={{ width: '300px', height: '340px', borderRadius: '10px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)' }}>
                <h3 style={{ color: '#1890ff', marginBottom: '10px', textAlign: 'center' ,fontWeight: 'bold', fontSize: '1.5rem' }}>{item.title}</h3>
                <hr />
                <div style={{ color: '#666', marginBottom: '10px', fontSize: '1rem' }}>
                  <strong>인원 현황: </strong> {item.currentNum} / {item.recruitNum}
                  <br />
                  <Progress percent={item.currentNum && item.recruitNum ? Math.floor((item.currentNum / item.recruitNum) * 100) : 0} status="active" />
                </div>
